...
class TestOptimization(unittest.TestCase):
    ...
    def test_mutation(self):
        ...
        offspring = mutation(
            offspring_crossover=population, num_mut_per_ele=4, mut_probability=0.5
        )
        new_scores = np.array([fitness_sum(_) for _ in offspring])
    ...
    def test_generation(self):
        ...
        for _ in range(ngen):
            offspring = crossover(parents=population, offspring_size=offspring_size)
            offspring = mutation(
                offspring_crossover=offspring, num_mut_per_ele=4, mut_probability=0.5
            )
        ...
        scores_ref = np.array(
            [87.0, 86.0, 84.0, 84.0, 84.0, 82.0, 82.0, 82.0, 82.0, 82.0]
        )
